// assets/js/content-legal-impressum.js
(function () {
  async function loadContent(lang) {
    const url = `../content/${lang}/legal/impressum.json`;
  ...
  function render(data) {
    ...
    // links
    const a = document.getElementById('legal-to-privacy');
    if (a) {
      a.textContent = data?.links?.toPrivacy ?? '';
      a.setAttribute('href', data?.links?.privacyHref ?? 'datenschutz.html');
    }
  }
  ...
  async function update(lang) {
    try {
      ...
      console.error('[content-legal-impressum] render failed:', e);
    }
  }
